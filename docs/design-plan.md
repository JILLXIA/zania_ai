...
flowchart TD
    ...
    R --> S[Prepare exact citation passages and allowed IDs]
    S --> L[Bounded async gpt-4o-mini answer and ID selection]
    L --> G[Validate IDs and assemble server-owned citations]
...
- Before the answer call, prepare server-owned citation passages from only the retrieved chunk text. Slice at up to 600 characters, preferring sentence ends and then word boundaries; do not rewrite text. Boundary searches start at least 80 characters into a window to bound the number of IDs. Long sentences may span multiple passages. Assign IDs such as `c33:p0`, retaining the original chunk and source metadata.
- Send the passages in source order, grouped by their original chunk with its context label. This preserves surrounding qualifiers and JSON question/answer relationships without duplicating the full chunk in the prompt. Context labels are not separately citable. Source text remains within the retrieval budget; passage labels and the enum schema add some prompt overhead.
- Ask for structured output with `status`, `answer`, `missing_details`, and `evidence_ids`. Use a fresh Pydantic schema whose ID enum contains only this question's supplied passages; reuse the existing model/client without mutating shared schemas. The model selects passages rather than generating quotations or source locations. Keep partial/missing-answer conventions unchanged. [OpenAI Structured Outputs](https://developers.openai.com/api/docs/guides/structured-outputs#supported-schemas)
- Resolve IDs against the question-local passage map and copy each passage's exact text, page/path, and `source_type` into citations on the server. Deduplicate repeated selections and retain a defensive source-substring check. The model cannot attach a rewritten quote to an otherwise valid ID or relabel a visual observation as native text. The public API's `citations` shape is unchanged.
- An answered/partial result must have at least one valid citation. Reject unavailable IDs, inconsistent server passages, contradictory output fields, refusals, and truncated/invalid outputs as per-question generation errors. The old free-text citation output format is not accepted. Do not disguise errors as missing document evidence.
- Citation resolution establishes provenance, not semantic entailment. A model can still select a real but irrelevant passage. Answer support still depends on the model and must be assessed with manual sample evaluation and adversarial fixtures; do not describe this as a hallucination guarantee. No repair model or fuzzy citation matching is added.
...
- Generation: supported, unsupported, and partially supported questions; partial answers explicitly describe missing details in their answer text; source `Yes`/`No` interpreted with record context; `Data-Not-Found` handling; question-local passage ID enums; unknown ID rejection; server-owned exact excerpts; forbidden legacy quote fields; preserved image/native provenance; conflicting text/visual evidence; invalid structured output; provider refusal; prompt injection in a question, text record, or depicted image text.
